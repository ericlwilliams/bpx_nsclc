...
%% TEMPORARY
%problem_pts = {'Bochicco' 'Carnevale' 'Dagai' 'Flores'...
%    'Grasso' 'Lefkowitz' 'Nadel' 'Coluccio' 'Demeyer'...
%    'Ferretti' 'Hohenshelt' 'Jones' 'Paleoyannis'...
%    'Petrone' 'Pomerance'};
problem_pts = {''};
...
pathname='Z:/elw/MATLAB/original_data/BPX/DVHs/';
...
h=cellfun(@(x) strcmp(x,'Dmax of Brachial Plexus (cGy)'),xlsraw(1,:)); h=find(h); h=h(1); % found the column of "Patient Last Name"
dmax=xlsraw(2:end,h); % patient last name

h=cellfun(@(x) strcmp(x,'Volume (cc)'),xlsraw(1,:)); h=find(h); h=h(1); % found the column of "Patient Last Name"
vmax=xlsraw(2:end,h); % patient last name

% Treatment site (use to find correct DVH file for duplicate entries)
g=cellfun(@(x) strcmp(x,'BPx Side'),xlsraw(1,:)); g=find(g); g=g(1); % found the column of "Patient Last Name"
...
for k=1:size(ptnames,1)
    ...
    % TEMPORARY
    problem = false;
    for j=1:length(problem_pts)
        if strcmp(ptname,problem_pts{j})
            problem=true;
            break;
        end
    end
    if problem, continue;end
...
%     
    ...
    if length(f)>1
        ...
        loc_str = BPxLocationString(ptname,txsites{k});
    ...
    disp(['Loading ',num2str(k),' ',dvh_loc,'...']);
    ...
    stairs(DVH{k,3}(:,1),DVH{k,3}(:,3),'r'); 
    plot([cur_dmax cur_dmax]./100, ylim,'b--','LineWidth',2);
    plot(xlim,[cur_vmax cur_vmax],'b--','LineWidth',2);
    hold off;
    grid on; pause(0);
    pt_added=pt_added+1;
...
% save result
    ...
    fn='Z:\elw\MATLAB\bpx_analy\meta_data\BPx_MASTER_DVHs.mat';
