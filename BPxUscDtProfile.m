...
%CGobj_current.mLymanN = [10^(-2);CGobj_current.mLymanN];

CGobj_current.mLymanN = 10.^(-1.9:0.1:0.1)';
...
% %% Set dose correction to USCBED, calculate USCBEDs
 for k=1:length([CGobj_current.mGrp])
     ...
     %CGobj_current.mGrp(k).mLymanN = [10^(-2);CGobj_current.mGrp(k).mLymanN];
     
     CGobj_current.mGrp(k).mLymanN = 10.^(-1.9:0.1:0.1)';
 end
 ...
 %mUscRangeAlphaD0 = 0.01:0.05:.7;
 
 
 %mUscRangeDt = 0.1:0.1:30;
 %mUscRangeAlphaD0 = 0.1:0.1:.7;
 
 %mUscRangeAlphaD0 = 0.05:0.05:.5;
 %mUscRangeAlphaD0 = 0.01:0.01:.5;
...
stepDt = 0.25; 
mUscRangeDt = 30:-stepDt:stepDt;
mUscRangeDq = max(mUscRangeDt)/2:-2*stepDt:2*stepDt; 
...
mUscBestLog10a = -inf(length(mUscRangeDt),length(mUscRangeDq));
mUscLogLikelihoods = -inf(length(mUscRangeDt),length(mUscRangeDq));
mUscLogLikelihoods68 = -inf(length(mUscRangeDt),length(mUscRangeDq));
mUscLogLikelihoods95 = -inf(length(mUscRangeDt),length(mUscRangeDq));
mUscAICs = -inf(length(mUscRangeDt),length(mUscRangeDq));
mUscDq = -inf(length(mUscRangeDt),length(mUscRangeDq));
mUscAlphaD0 = -inf(length(mUscRangeDt),length(mUscRangeDq));
mUscPvals = -inf(length(mUscRangeDt),length(mUscRangeDq));
mUscFracLQ = -inf(length(mUscRangeDt),length(mUscRangeDq));
mUscFracFullLQ = -inf(length(mUscRangeDt),length(mUscRangeDq));
 ...
 for i=1:length(mUscRangeDt);
     ...
     j=1;
     for cur_dq=cur_dt/2:-2*stepDt:stepDt;
